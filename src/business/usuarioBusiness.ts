...
export class UserBusiness {
    ...
    public async createUser(input: Omit<User, "id_usuario" | "data_criacao">): Promise<User> {
        try {
            ...
            const hashedPassword = await AuthBusiness.hashPassword(input.senha);
    ...
    public async updateUser(
        id_usuario: number,
        input: Omit<User, "id_usuario" | "data_criacao">
    ): Promise<void> {
        try {
            ...
            if (userWithSameEmail && userWithSameEmail.id_usuario !== id_usuario) {
                throw new Error("Email ja cadastrado por outro usuario.");
            }

            const hashedPassword = await AuthBusiness.hashPassword(input.senha);

            const updatedUser: Omit<User, "id_usuario" | "data_criacao"> = {
                ...input,
                senha: hashedPassword,
            };
    ...
    public async deleteUser(id_usuario: number): Promise<void> {
        try {
            ...
            if (user.tipo === "ADMIN") {
                throw new Error("A exclusao de um usuario Administrador nao e permitida.");
            }
